...
public static class FilmRoutes
{
  public static void start()
  {
    ...
    App.MapGet("/api/films", (HttpContext context) =>
    {
      ...
      string ageLimit = query.TryGetValue("ageLimit", out a) ? a.ToString() : null;
    ...
    // GET/api/films/{id}
    AppContext.MapGet("/api/films/{id}", (HttpContext context, int id) =>
    {
      var sql = @"
      ...
      {
        return RestResult.Parse(context, new { error = "Film not found." });
      }
